...
namespace Dispatcher
{
    class Dispatcher
    {
        private ServersComunicator servers; // колекция соединений с серверами
        ...
        static void Main(string[] args)
        {
            ...
            /*
            ServersComunicator servComunicator = new ServersComunicator(8777);
            servComunicator.Start();
            */

            Broadcaster broadcasterObj = new Broadcaster(1700, "239.254.255.255");
            broadcasterObj.Start();
        }
    }
    ...
    class Broadcaster
    {
        ...
        public Broadcaster(int Port, string remoteIPAdr)
        {
            remotePort = Port;
        ...
        public void Start()
        {
            Thread broadcastThread = new Thread(BroadcastSendProcess);
        ...
        private void BroadcastSendProcess()
        {
            ...
            try
            {
                while (true)
                {
                    ...
                    string message = String.Format("{0} time: {1}:{2}:{3}", broadcastMessage, dt.Hour, dt.Minute, dt.Second);
